...
const SupportRequestPage: React.FC = () => {
      ...
      try {
        const endpoint =
          currentRole === 'PROVIDER'
            ? `http://localhost:8080/api/bookings/provider/${currentUserId}`
  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/support/tickets/user/${currentUserId}`);
        const data = await response.json();
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}>
        ...
        >
          ...
          <Typography variant="body1" sx={{ opacity: 0.9 }}>
            Fast support for booking, service, payment, or account queries. We respond quickly.
          </Typography>
        </Card>
